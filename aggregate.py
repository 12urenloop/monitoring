...
# pprint(sorted(count_laps, key=lambda l: l["combi_lapper"]))
pprint(count_laps)

# print(count_laps[list(teams.values())])
team_names = list(map(lambda a: a['name'],
                      sorted(teams.values(), key=lambda a: count_laps.get(a['name'], dict()).get('combi_lapper', 0),
                             reverse=True)))
